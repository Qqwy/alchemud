...
#   defstart start(initial_state = %Location.State{module: _, uuid: uuid}), gen_server_opts: [name: {:global, {:location, uuid}}] do
...
#   defcall exits, state: %Location.State{exits: exits}, do: reply(exits) 

#   defhandleinfo :tick, state: state = %Location.State{module: location_module} do
...
#   defcast add_exit(pid, way = %Way{name: exit_name}), state: state = %Location.State{ways: ways, exits: exits} do
#     IO.puts "Adding exit:"
#     IO.inspect pid
#     IO.inspect way
#     IO.inspect state
...
#   def add_incoming_ways(state = %Location.State{ways: ways, uuid: uuid}) do
